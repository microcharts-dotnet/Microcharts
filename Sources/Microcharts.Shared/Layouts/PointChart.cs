namespace Microcharts
{
	...
	public class PointChart : Chart
	{
		...
		{
			if (this.MaxValue <= 0) return headerHeight;
			if (this.MinValue > 0) return headerHeight + itemHeight;
					...
					})
					{
						switch (this.PointMode)
						{
							case PointMode.Square:
								canvas.DrawRect(SKRect.Create(point.X - this.PointSize / 2, point.Y - this.PointSize / 2, this.PointSize, this.PointSize), paint);
								break;
