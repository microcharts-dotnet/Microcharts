namespace Microcharts
{
	...
	public class RadialGaugeChart : Chart
	{
		...
		private float AbsoluteMinimum => this.Entries.Select(x => x.Value).Concat(new[] { this.MaxValue, this.MinValue, this.minValue ?? 0 }).Min(x => Math.Abs(x));

		private float AbsoluteMaximum => this.Entries.Select(x => x.Value).Concat(new[] { this.MaxValue, this.MinValue, this.minValue ?? 0 }).Max(x => Math.Abs(x));
